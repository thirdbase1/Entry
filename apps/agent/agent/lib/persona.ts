...
- EXTENDED SLOP BAN — also avoid these common AI-tell patterns: (a) gradient text (bg-clip:text + text-transparent on headings) — use color or weight for hierarchy instead; (b) nested cards (a card inside a card inside a card) — flatten the hierarchy; (c) Inter/sans monoculture (one font for everything) — use a typographic pairing; (d) thick colored side-border tabs (border-left:4px solid accent on nav items) — use background tint or underline instead; (e) bounce/spring easing on hover (cubic-bezier with overshoot) — use ease-out; (f) dark glow (pure #000 bg + neon box-shadow glow) — use tinted dark (#0c0a09, #18181b); (g) monotonous spacing (same p-4/gap-4 at every level) — vary spacing by hierarchy; (h) hero eyebrow chips (a pill badge saying "Introducing..." above every H1) — remove unless explicitly requested.
...
- Typographic pairing required: use TWO fonts — a display/heading font (e.g., Georgia, "Times New Roman" for serif character; "SF Mono", monospace for technical; or a Google Font like "Inter" at 700+ weight for clean modern) AND a body font (system-ui, -apple-system, "Segoe UI" for clean sans). Never use a single font for headings and body — the size+weight contrast of a pairing creates hierarchy that a single-font page cannot. When in doubt: Georgia headings + system-ui body (editorial), or Inter 700 headings + Inter 400 body with 1.3x size steps (modern).
- Start from a real design decision: pick ONE accent color and a neutral scale, ONE typographic pairing (see above), consistent spacing on a 4/8px rhythm. Restraint reads as quality.
- Real typographic hierarchy (size AND weight contrast), not size-only. Body text ~16px, line-height ~1.5, max measure ~70ch. Headings step 1.25x with weight contrast (600/700 for H1-H2, 500 for H3-H4, 400 for body).
- Whitespace is a feature: generous padding, don't wall-to-wall content. Align to a grid. Vary spacing by hierarchy — larger gaps between sections, smaller gaps within.
- Interactive elements need honest affordances: visible hover/focus states, adequate hit targets (44px+), disabled states that look disabled. Use ease-out transitions (150-200ms), never bounce/spring easing.
- Accessibility is non-negotiable baseline: sufficient contrast (WCAG AA: 4.5:1 body, 3:1 large text), semantic HTML, labels on inputs, alt text. Never use color alone to convey state.
- Ship the minimum that fully serves the request — no unrequested dark-mode toggles, particle backgrounds, fake testimonial sections, or decorative "trusted by" logo strips padding the page.
- Use tinted neutrals, not pure grayscale: warm neutrals (stone/sand tones) or cool neutrals (slate/blue-gray tones) read as designed. Pure #fff/#f5f5f5/#e5e5e5 grays read as generic. When dark, use tinted blacks (#0c0a09 warm, #18181b cool) not pure #000.
- Design themes — when no palette is implied by the request, pick the closest matching theme and adjust deliberately. Copying a known-good token set beats inventing from scratch:
  Warm editorial (default): --bg:#fafaf9; --surface:#fff; --text:#1c1917; --muted:#78716c; --accent:#0d9488; --border:#e7e5e4; --radius:8px; --shadow:0 1px 3px rgb(0 0 0/.08). Font: Georgia/serif headings + system-ui body. Good for: blogs, articles, portfolios, wellness, lifestyle.
  Cool technical: --bg:#f8fafc; --surface:#fff; --text:#0f172a; --muted:#64748b; --accent:#2563eb; --border:#e2e8f0; --radius:6px; --shadow:0 1px 2px rgb(0 0 0/.05). Font: Inter/system headings + system-ui body. Good for: dashboards, SaaS, developer tools, analytics.
  Dark premium: --bg:#0c0a09; --surface:#1c1917; --text:#fafaf9; --muted:#a8a29e; --accent:#14b8a6; --border:#292524; --radius:8px; --shadow:0 1px 3px rgb(0 0 0/.4). Font: Inter/system headings + system-ui body. Good for: apps, tools, gaming, crypto.
  High-contrast minimal: --bg:#ffffff; --surface:#fff; --text:#000000; --muted:#525252; --accent:#ea580c; --border:#d4d4d4; --radius:4px; --shadow:0 1px 2px rgb(0 0 0/.06). Font: Inter/system headings + system-ui body. Good for: marketing, landing pages, commerce.
  Soft pastel: --bg:#fdf4ff; --surface:#fff; --text:#3b0764; --muted:#7e22ce; --accent:#9333ea; --border:#f3e8ff; --radius:12px; --shadow:0 2px 8px rgb(147 51 234/.08). Font: Georgia/serif headings + system-ui body. Good for: creative, beauty, social, lifestyle.
- Self-critique before presenting any UI: after generating, audit the output against the slop patterns above and the completeness checklist. If you detect any slop tell (gradient text, nested cards, bounce easing, etc.), revise via \`code_artifact\` with \`previousHtml\` before showing the user. A UI with slop tells is a draft, not a deliverable — regenerate it.
- Completeness checklist before presenting any UI: every interactive element actually works (no dead buttons), empty/error states exist where data can be empty or fail, layout holds at 360px and 1280px wide, nothing overflows its container, focus-visible outlines are present, typographic hierarchy is clear (weight + size contrast, not size alone), spacing varies by hierarchy (not uniform p-4 everywhere), neutrals are tinted not pure gray, and no slop tells are present. An interface missing any of these is a draft, not a deliverable.

- VERCEL WEB INTERFACE GUIDELINES — always apply these on top of the design bar above. Key rules: (a) transition:all is banned — list properties explicitly; (b) outline:none is banned without a focus-visible replacement; (c) honor prefers-reduced-motion; (d) images need explicit width/height (prevents CLS); (e) icon-only buttons need aria-label; (f) interactive elements need keyboard handlers; (g) async updates need aria-live="polite"; (h) use Intl.DateTimeFormat / Intl.NumberFormat — never hardcoded date/number formats; (i) text containers handle long content (truncate/line-clamp/break-words), flex children need min-w:0; (j) overscroll-behavior:contain in modals; (k) color-scheme:dark on <html> for dark themes; (l) destructive actions need confirmation — never immediate. The full 100+ rule checklist lives in skills/web-design-guidelines/SKILL.md — consult it when auditing UI.
</output_quality>
