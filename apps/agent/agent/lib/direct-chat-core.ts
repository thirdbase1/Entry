 ...
 */
// PXXL PORT (2026-07-20): this is the whole reason this file exists as a
// SECOND copy of apps/web/app/api/direct/chat/route.ts, running inside
// apps/agent's own long-lived Pxxl process instead of a Vercel Serverless
// Function -- there is no maxDuration ceiling here at all; the container
// just keeps running for as long as the turn genuinely takes. See
// direct-chat.ts (the channel wrapper in this same directory) for the
// HTTP/auth seam -- this file is the ported turn body, kept as close to
// line-for-line identical to the original as possible so the two stay
// diffable against each other as fixes land on either side.
import {
  streamText,
  tool,
  stepCountIs,
...
/**
 * PXXL PORT (2026-07-20): entry point called by ../channels/direct-chat.ts
 * (the actual HTTP/JWT-auth seam) after it has already verified the
 * caller and resolved `userId` — no session/cookie handling happens in
 * here at all, unlike the original Vercel route this was ported from.
 * `waitUntil` is threaded in from the channel's own route helpers
 * (eve's custom-channel API) instead of `@vercel/functions`' version —
 * same "extend the request lifetime for background work" contract, just
 * sourced from the host that's actually running this request.
 */
export async function runDirectChatTurn(
  userId: string,
  body: unknown,
  waitUntil: (p: Promise<unknown>) => void,
): Promise<Response> {
  const { id, messages, byokModelId, requestedModel, disabledTools } = (body ?? {}) as {
    id?: string; messages?: unknown[]; byokModelId?: string; requestedModel?: string; disabledTools?: unknown;
  };
  ...
  const result = streamText({
    ...
    repairToolCall: async ({ toolCall, tools, error }) => {
      ...
      // PORTED (2026-07-20, from apps/web/app/api/direct/chat/route.ts's
      // same fix -- real bug: write_file threw a raw Zod "expected:
      // string, received undefined, path: ['path']" straight to the
      // user). Models sometimes call write_file/read_file/append_file/
      // edit_file with `file_path`/`filePath`/`filename`/`fileName`
      // instead of the real, required `path` key. That's an
      // InvalidToolInputError (schema validation failure on otherwise
      // well-formed JSON), not a NoSuchToolError, so it needs its own
      // repair branch: parse the raw input, and if `path` is missing but
      // exactly one known alias is present, rename it and let the SDK
      // re-validate. Never invents a value that wasn't in the original
      // call -- returns null (original error surfaces normally) whenever
      // there's no such alias to rescue.
